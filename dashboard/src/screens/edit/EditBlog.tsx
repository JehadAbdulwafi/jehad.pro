...
const EditBlog = () => {
  ...
  useEffect(() => {
    ...
    const getBlog = async () => {
      try {
        ...
        const response = await axiosClient.get(`/posts/${id}`, { signal });
        const data = response.data as Blog;
        if (response && response.status === 200) {
          setCustomeValue("name", data.name);
          setCustomeValue("jobTitle", data.jobTitle);
          setCustomeValue("title", data.title);
          setCustomeValue("desc", data.desc);
          setCustomeValue("category", data.category);
          setCustomeValue("content", data.content);
          setCustomeValue("image", data.image);
          setCustomeValue("readTime", data.readTime);
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    ...
    try {
      setState(STATE.LOADING);
      const response = await axiosClient.post(`/posts/${id}`, data, {
        headers: {
